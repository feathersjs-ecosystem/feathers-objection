...
class Service extends _adapterCommons.AdapterService {
  constructor(options) {
    ...
    this.allowedEager = options.allowedEager && _objection.RelationExpression.create(options.allowedEager);
  ...
  createQuery(params = {}) {
    ...
    q.allowEager(allowEager || NO_RELATIONS); // $select uses a specific find syntax, so it has to come first.
    ...
    if (query && query.$joinEager) {
      q.eagerAlgorithm(this.Model.JoinEagerAlgorithm).eager(query.$joinEager, this.namedEagerFilters);
      delete query.$joinEager;
    }

    if (query && query.$joinRelation) {
      q.distinct(`${this.Model.tableName}.*`).joinRelation(query.$joinRelation);
    ...
    const find = (params, count, filters, query) => {
      ...
      if (count) {
        ...
        if (query.$joinRelation) {
          countQuery.joinRelation(query.$joinRelation).countDistinct({
            total: idColumns
          });
  ...
  _update(id, data, params) {
    ...
    return this._get(id, params).then(oldData => {
      ...
      if (allowedUpsert) {
        return this._createQuery(params).allowUpsert(allowedUpsert).upsertGraphAndFetch(newObject, this.upsertGraphOptions);
  ...
  _patch(id, data, params) {
    ...
    if (allowedUpsert && id !== null) {
      ...
      return this._createQuery(params).allowUpsert(allowedUpsert).upsertGraphAndFetch(dataCopy, this.upsertGraphOptions);
    }
